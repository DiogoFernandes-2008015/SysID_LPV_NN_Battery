...
def init_wnn_params(input_size, num_wnn_neurons, output_size, key, scale=1e-2):
    """
    Inicializa os parâmetros da Rede Neural Wavelet (WNN).
    Parâmetros: Translações (B), Log-Escalas (log_A) e Pesos de Saída (W_out, b_out).
    """
    b_key, a_key, w_key, b_key_out = random.split(key, 4)

    # 1. Translações (B): [num_wnn_neurons, input_size]. (Equivalente aos Centros)
    B = random.uniform(b_key, (num_wnn_neurons, input_size), minval=0.0, maxval=1.0)

    # 2. Log-Escalas (log_A): [num_wnn_neurons]. (Equivalente a log_sigma)
    # Valores iniciais próximos de zero geram A ≈ 1 (boa escala inicial)
    log_A = 0.1 * random.normal(a_key, (num_wnn_neurons,))

    # 3. Pesos de Saída (W_out) e Bias (b_out) são iguais
    W_out = scale * random.normal(w_key, (output_size, num_wnn_neurons))
    b_out = scale * random.normal(b_key_out, (output_size,))

    return [B, log_A, W_out, b_out]

def morlet_wavelet(x, b, log_a):
    """
    Função de Ativação Wavelet de Morlet (Real).
    psi(t) = exp(-t^2/2) * cos(5t) (exemplo comum de Morlet)
    Aqui: psi( (x - b) / a )

    Parâmetros:
    x (jnp.array): Vetor de entrada (e.g., [SOC]).
    b (jnp.array): Vetor de translação.
    log_a (jnp.array): log do parâmetro de escala (dilatação), a.
    """
    a = jnp.exp(log_a) + 1e-6  # Garante escala positiva

    # Argumento da wavelet: (x - b) / a
    t = jnp.sum(x - b) / a

    # Parâmetro de frequência central (w0), 5 é comum
    w0 = 5.0

    # psi(t) = exp(-t^2/2) * cos(w0 * t)
    return jnp.exp(-0.5 * t ** 2) * jnp.cos(w0 * t)


def mexican_hat_wavelet(x, b, log_a):
    """
    Função de Ativação Wavelet Mexicana (Mexican Hat/Ricker).
    MexicanHat(t) = (1 - t^2) * exp(-t^2/2), onde t = (x - b) / a.
    """
    a = jnp.exp(log_a) + 1e-6  # Escala (dilatação)

    # x e b são vetores; a é um escalar (por neurônio).
    t = jnp.sum(x - b) / a

    # Mexican Hat
    return (1.0 - t ** 2) * jnp.exp(-0.5 * t ** 2)


if ACTIVATION_NAME == 'morlet':
    activation_fn = morlet_wavelet
if ACTIVATION_NAME == 'hat':
    activation_fn = mexican_hat_wavelet


def predict_wnn(params, inputs):
...
def hybrid_battery_1rc_jax(t, x, args):
    ...
    delta_R0, delta_R1, delta_C1 = predict_wnn(params_nn, nn_input)
...
initial_params_rbfn = init_wnn_params(input_size, rbf_neurons, output_size, key)
