...
def hybrid_battery_pngv_jax(t, x, args):
    ...
    delta_R0, delta_C0 ,delta_R1, delta_C1, delta_R2, delta_C2 = predict(params_nn, nn_input)
    R0 = 0.2440 * (1 + delta_R0)
    C0 = 43794.325*(1 + delta_C0)
    R1 = 49954.9339 * (1 + delta_R1)
    C1 = 3861.2425 * (1 + delta_C1)
    R2 = 50006.7429* (1 + delta_R2)
    C2 = 13360.366 * (1 + delta_C2)

    dx = [-0.337038*u / 3440.05372,
          u / C0,
          -1 / R1 / C1 * x[2] + 1 / C1 * u,
          -1 / R2 / C2 * x[3] + 1 / C2 * u]
    dx = jnp.array(dx)
...
term = ODETerm(hybrid_battery_pngv_jax)
...
nn_layer_sizes = [1, 32, 6]  # 1 inputs (SOC), 1 hidden layers of 64, 3 outputs
solver = Dopri5()
...
def objective_jax_nn(decision_vars):
    ...
    def model_output_step(t, x_step, u_interp_obj):
        ...
        y_pred_step = OCV + R0 * u + x_step[1]+x_step[2]+x_step[3]
...
delta_R0_vec, delta_C0_vec, delta_R1_vec, delta_C1_vec, delta_R2_vec, delta_C2_vec = vmap_calculate_deltas(soc_values_jax, params_nn_est)
...
R0_nominal = 0.2440
C0_nominal = 43794.325
R1_nominal = 49954.9339
C1_nominal = 3861.2425
R2_nominal = 50006.7429
C2_nominal = 13360.3664
# 5. Calcular os parâmetros absolutos
R0_actual = R0_nominal * (1 + delta_R0_vec)
C0_actual = C0_nominal * (1 + delta_C0_vec)
R1_actual = R1_nominal * (1 + delta_R1_vec)
C1_actual = C1_nominal * (1 + delta_C1_vec)
R2_actual = R2_nominal * (1 + delta_R2_vec)
C2_actual = C2_nominal * (1 + delta_C2_vec)
...
# Plot R1
axs[1].plot(soc_values_np, C0_actual, 'g-', linewidth=2)
axs[1].axhline(C0_nominal, color='k', linestyle='--', alpha=0.6, label='Valor Nominal')
...
axs[3].plot(soc_values_np, C1_actual, 'b-', linewidth=2)
axs[3].axhline(C1_nominal, color='k', linestyle='--', alpha=0.6, label='Valor Nominal')
axs[3].set_ylabel('$C_1$ (F)')
axs[3].set_xlabel('Estado de Carga (SOC)')
axs[3].set_title('Capacitância de Polarização ($C_1$)')
axs[3].grid(True)
axs[3].legend()
