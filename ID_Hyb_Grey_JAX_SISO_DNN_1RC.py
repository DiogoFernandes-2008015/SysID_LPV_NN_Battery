...
def hybrid_battery_1rc_jax(t, x, args):
    params_nn_R0, params_nn_R1, params_nn_C1, u_interp = args
    ...
    delta_R0 = predict(params_nn_R0, nn_input)[0]
    delta_R1 = predict(params_nn_R1, nn_input)[0]
    delta_C1 = predict(params_nn_C1, nn_input)[0]
    R0 = 0.268 * (1 + delta_R0)
...
nn_layer_sizes_R0 = [1, 16, 1]  # 1 inputs (SOC), 1 hidden layers of 64, 3 outputs
nn_layer_sizes_R1 = [1, 16, 1]  # 1 inputs (SOC), 1 hidden layers of 64, 3 outputs
nn_layer_sizes_C1 = [1, 16, 1]  # 1 inputs (SOC), 1 hidden layers of 64, 3 outputs
solver = Dopri5()

# --- Create Initial Guess and Parameter Structures ---
key = random.key(0)
initial_params_nn_R0 = init_network_params(nn_layer_sizes_R0, key)
initial_params_nn_R1 = init_network_params(nn_layer_sizes_R1, key)
initial_params_nn_C1 = init_network_params(nn_layer_sizes_C1, key)
# Store the structure of the NN parameters for later unflattening
flat_initial_nn_params_R0, params_nn_struct_R0 = ravel_pytree(initial_params_nn_R0)
len_nn_params_R0 = len(flat_initial_nn_params_R0)
flat_initial_nn_params_R1, params_nn_struct_R1 = ravel_pytree(initial_params_nn_R1)
len_nn_params_R1 = len(flat_initial_nn_params_R1)
flat_initial_nn_params_C1, params_nn_struct_C1 = ravel_pytree(initial_params_nn_C1)
len_nn_params_C1 = len(flat_initial_nn_params_C1)
...
initial_guess_np = np.concatenate([
    np.array(flat_initial_nn_params_R0),
    np.array(flat_initial_nn_params_R1),
    np.array(flat_initial_nn_params_C1),
...
def objective_jax_nn(decision_vars):
    ...
    params_nn_R0 = params_nn_struct_R0(decision_vars[:len_nn_params_R0])
    params_nn_R1 = params_nn_struct_R1(decision_vars[len_nn_params_R0:len_nn_params_R0+len_nn_params_R1])
    params_nn_C1 = params_nn_struct_C1(decision_vars[len_nn_params_R0+len_nn_params_R1:len_nn_params_C1 + len_nn_params_R0 + len_nn_params_R1])
    x_initial_shots = decision_vars[len_nn_params_R0+len_nn_params_R1+len_nn_params_C1:].reshape(n_shots, n_states)

    # args = (params_nn, u_interp)

    def simulate_shot(t_shot, w0):
        saveat = SaveAt(ts=t_shot)
        args = (params_nn_R0,params_nn_R1,params_nn_C1, u_interpolation)
    ...
    def model_output_step(t, x_step, u_interp_obj):
        ...
        delta_R0 = predict(params_nn_R0, nn_input)
        R0 = 0.268 * (1 + delta_R0)
    ...
    # Loss function
    return jnp.sum((y_pred_squeezed - y_data) ** 2)


@jit
def continuity_constraints_jax_nn(decision_vars):
    params_nn_flat_R0 = decision_vars[:len_nn_params_R0]
    params_nn_R0 = params_nn_struct_R0(params_nn_flat_R0)
    params_nn_flat_R1 = decision_vars[len_nn_params_R0:len_nn_params_R0+len_nn_params_R1]
    params_nn_R1 = params_nn_struct_R1(params_nn_flat_R1)
    params_nn_flat_C1 = decision_vars[len_nn_params_R0+len_nn_params_R1:len_nn_params_R0+len_nn_params_R1+len_nn_params_C1]
    params_nn_C1 = params_nn_struct_C1(params_nn_flat_C1)
    x_initial_shots = decision_vars[len_nn_params_R0+len_nn_params_R1+len_nn_params_C1:].reshape(n_shots, n_states)
    args = (params_nn_R0,params_nn_R1,params_nn_C1, u_interpolation)
...
params_nn_est_R0 = params_nn_struct_R0(result.x[:len_nn_params_R0])
params_nn_est_R1 = params_nn_struct_R1(result.x[len_nn_params_R0:len_nn_params_R1+len_nn_params_R0])
params_nn_est_C1 = params_nn_struct_C1(result.x[len_nn_params_R0+len_nn_params_R1:len_nn_params_R0+len_nn_params_R1+len_nn_params_C1])

# Use the first identified shot state as the initial state for the full simulation
x_initial_estimated = jnp.array(result.x[len_nn_params_R0+len_nn_params_R1+len_nn_params_C1:len_nn_params_R0+len_nn_params_R1+len_nn_params_C1 + n_states])

print("\n--- Identification Results ---")

# --- Time-Domain Validation Plot ---
final_args = (params_nn_est_R0,params_nn_est_R1,params_nn_est_C1, u_interpolation)
...
def model_output_step(t, x_step, params_nn_R0, u_interp_obj):
    u = u_interp_obj.evaluate(t)
...
y_hat = jax.vmap(model_output_step, in_axes=(0, 0, None, None))(jnp.array(time), final_sol.ys, params_nn_est_R0,
                                                                u_interpolation)
# Metrics
